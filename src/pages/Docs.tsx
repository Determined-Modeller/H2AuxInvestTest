...
The H2 AuxInvest project aims to solve the problem of poor knowledge transfer in Scotland's Hydrogen Supply Chain by providing an open-source tool. 
...
The placeholder assumption here is of a hydrogen pipeline connected to hydrogen distribution piping, delivering hydrogen at up to 7 bar. 
Should this instead be via tube trailer, please choose the specific tube trailer delivery pressure, 
usually available on a public datasheet or by request.


If you are still unsure about what pressure to choose, but know you will not be connected to the gas distribution network, 
it is sensible to start with one of the following commonly used tube trailer pressures in industry:
...
The parameters of the hydrogen demand (i.e., tank capacity and dispensing pressure of the vehicles) are crucial for specifying the correct hydrogen infrastructure. 
Please see the vehicle/trailer datasheet, or contact the vendor to confirm that this is correct.
...
Storage makes up a significant portion of the final cost of any hydrogen supply infrastructure, in this section, 
you specify what storage infrastructure you wish to use, outside of the fuelling cascade required to meet peak demand (when this is applicable).

If you are directly connected to a part of the national gas grid, in an area where hydrogen will be usable you are advised to not include storage (unless operational integrity is a priority). 
...
The distribution of hydrogen demand is an important part of accurately specifying and costing any hydrogen infrastructure. 
It is recommended that when finally designing hydrogen infrastructure, that this is completed, using expected/measured delivery and filling schedules, traffic data, vendor information, and other sources.

However, for the purpose of shortcut estimation, as this information is rarely available during early estimation stages, peak and average supply rates can be used with a "standard" normal distribution.
Application of more sophisticated options for a probabilistic approach, incorporating different standard profiles, considering queue theory and maximum number of back-to-back fills is an ongoing development task for the project.

If you are unsure about this demand we suggest specifying this using the "vehicles per hour/vehicles per day" units options, specifying your peak demand by the highest number of fills you require during an hour/day. 
You should then select the average demand by selecting the "number of vehicles per hour/vehicles per day", filling in what you expect to be the average demand level.

If you are looking to explore limitations on pressure selection please consider application of standard values from SAEJ2601. 
...
This can be used to estimate the upper bound of the peak flowrate demand.

# **Detailed Configuration**
==

The Weighted Average Cost of Capital (WACC) represents a firm's blended cost of capital across all sources, including debt and equity, weighted by their respective proportions in the company's capital structure. 
Choosing a sensible WACC value is crucial for evaluating investment decisions and calculating the net present value (NPV) of future cash flows, where accuracy in estimating the cost components and their weights directly impacts investment appraisal outcomes.

Sensible selection of WACC involves careful consideration of the current market conditions, the firm's capital structure, and risk factors, ensuring that the chosen rate realistically reflects the company's cost of financing. 
...
The cost of equity can be estimated using models such as the Capital Asset Pricing Model (CAPM), which considers the risk-free rate, the market risk premium, and the company's beta (a measure of its volatility compared to the market).
...
Precooling is used to refrigerate hydrogen coming out of storage to achieve a higher number of back-to-back fills and higher State of Charge "SoC" in the hydrogen tanks, 
...
as per the standard SAEJ2601. There is currently no ability to change this assumption in the tool, which is an active development task.
...
The methodology incorporates real-world data and supplier-agnostic information to provide a balanced view of new vs. established technologies. 
The functions to calculate this are provided in the project folder. If you notice an issue/wish to improve a function, please raise an issue on the project github and/or send a push request.

The following python modules handle the engineering calculations:
...
Outlined below are two utility functions used for converting mass flow rates and vehicle refueling rates into standardized units of kilograms per hour (kg/h). 
These functions facilitate the handling of various input units, ensuring consistent calculations across different scenarios.
...
This parent class is designed as a base for specific compressor types and is not intended to be instantiated directly. 
It implements common calculations and properties shared among different compressor types to avoid duplicating code.
...
- **calculate_storage_equipment_cost()**: Custom equipment cost calculation for Type III/IV storage systems. Note: The provided method's documentation mentions it's identical to the base class's implementation. 
If there's no distinct calculation for Type III/IV, this method may not need to be overridden.
...
The class includes several assumptions about refuelling rates and additional time required for connection/disconnection, which are used to determine refuelling times. 
These values and the specific calculations depend on whether precooling is used and the type of vehicle being refuelled (tube trailer or passenger vehicle).

Cost calculations are influenced by the specified dispensing pressure, affecting equipment cost, installation cost, and energy cost. 
Maintenance costs are assumed to be a percentage of the equipment cost, and all costs contribute to the calculation of the levelized cost of hydrogen (LCOH).

*Usage*

Instantiate the **Dispenser** class with the average and peak hydrogen flow rates and a dictionary of input parameters. 
Use the **calculate_all()** method to perform all calculations and access the **results** attribute to retrieve detailed cost and operational information.


# **LCOH Calculator**
--

The **H2AuxCostCalculator** class is designed to facilitate comprehensive cost analysis for hydrogen supply chain hardware. 
It integrates functionalities for converting flow rates and pressures, instantiating hardware components, performing necessary calculations, and compiling cost summaries.
...
Instantiate the class and call the **calculate_costs** method with the user input parameters as a dictionary. 
The method returns a detailed dictionary containing cost analysis for each piece of hardware involved in the hydrogen supply chain.
...
The tool's accuracy is dependent on the quality and completeness of the input data. Assumptions are made regarding standard operational conditions and calculation workflow, 
it is recommended to consult with specialists for critical decisions. The tool is continuously updated to reflect new data and user feedback, yet it may not capture all nuances of every unique project scenario.
    `;
