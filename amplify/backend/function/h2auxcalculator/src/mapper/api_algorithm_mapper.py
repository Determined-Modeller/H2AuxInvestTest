...
class ApiAlgorithmMapper:
    ...
    def requestToAlgInput(self, request: RequestSchema) -> dict:
        ...
        if response['is_storage_required']:
            response['storage_requirement'] = request['storage_mass']['value'] if isinstance(request['storage_mass']['vsalue'], (int, float)) else 0
            ...
            response['storage_requirement'] = 0
            response['storage_requirement_unit'] =  'kg'
            response['storage_pressure'] =  0
            response['storage_pressure_unit'] = 'bar'
